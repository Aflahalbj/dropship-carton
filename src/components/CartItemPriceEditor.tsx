...
const CartItemPriceEditor: React.FC<CartItemPriceEditorProps> = ({
  ...
  return <div className="mt-2 pl-2">
      <div className="flex items-center space-x-2 mb-1">
        <Checkbox id={`enable-price-change-${productId}`} checked={isEnabled} onCheckedChange={handleToggleChange} />
        <label htmlFor={`enable-price-change-${productId}`} className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          Ubah harga sementara
        </label>
      </div>
      
      {isEnabled && <div className="flex items-center mt-1">
          <span className="text-sm text-muted-foreground mr-2">Rp</span>
          <Input type="text" className="w-32 h-8 text-sm" value={price} onChange={handlePriceChange} onBlur={handlePriceBlur} />
        </div>}
    </div>;
};
