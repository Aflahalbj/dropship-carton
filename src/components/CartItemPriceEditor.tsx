...
const CartItemPriceEditor: React.FC<CartItemPriceEditorProps> = ({
  ...
  return <div className="flex flex-col space-y-1 mt-1">
      <div className="flex items-center space-x-2">
        <Checkbox id={`enable-price-change-${productId}`} checked={isEnabled} onCheckedChange={handleToggleChange} className="mr-2" />
        <Label htmlFor={`enable-price-change-${productId}`} className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 truncate my-0 py-[2px]">
          Ubah harga sementara
        </Label>
      </div>
      
      {isEnabled && <div className="flex items-center ml-6">
          <span className="text-sm text-muted-foreground mr-1">Rp</span>
          <Input type="text" className="w-24 h-7 text-sm" value={price} onChange={handlePriceChange} onBlur={handlePriceBlur} />
        </div>}
    </div>;
};
